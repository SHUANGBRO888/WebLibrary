...
export const AdminMessages = ()=>{
    ...
    async function submitResponseToQuestion(id: number, reponse: string){
        const url = 'http://localhost:8080/api/messages/secure/admin/message';
        if(authState && authState?.isAuthenticated && id != null && reponse !== ''){
            const messageAdminRequestModel: AdminMessageRequest = new AdminMessageRequest(id, reponse);
            ...
            const messageAdminResponseModelResponse = await fetch(url, requestOptions);

            if(!messageAdminResponseModelResponse.ok){
                throw new Error('Something went wrong!')
            }
